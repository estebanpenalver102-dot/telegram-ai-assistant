...
# Model ladder: try each in order if rate-limited
MODEL_LADDER = [
...
# -- Context management: never hit token limits --
    ...
    try:
        summary_resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "\n".join(m["role"].upper() + ": " + m["content"] for m in to_summarize)},
            ],
            max_tokens=600,
        )
        summary_text = summary_resp.choices[0].message.content.strip()
        summarized_msg = {"role": "system", "content": "[Project context summary]\n" + summary_text}
        state = user_state[user_id]
        proj  = state["active"]
        state["projects"][proj] = [summarized_msg] + recent
        logger.info("Summarized history for user %s, project '%s'", user_id, proj)
    except Exception as e:
        logger.warning("Summarization failed: %s -- trimming instead", e)
        state = user_state[user_id]
...
def call_with_fallback(messages: list, max_tokens: int = 1500):
    ...
    for model in MODEL_LADDER:
        try:
            ...
            logger.warning("Rate limit on %s: %s. Trying next.", model, e)
            last_err = e
            time.sleep(1)
        except APIStatusError as e:
            if e.status_code == 429:
                logger.warning("429 on %s: %s. Trying next.", model, e)
                last_err = e
                time.sleep(1)
            else:
                raise
...
def _wikipedia(query: str) -> str:
    try:
        r = httpx.get(
            ...
            + httpx.utils.quote(query) + "&format=json&srlimit=1",
            timeout=8,
        )
        ...
        r2 = httpx.get(
            "https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=true&explaintext=true&titles="
            + httpx.utils.quote(title) + "&format=json",
            timeout=8,
        )
        ...
        return "Wikipedia error: " + str(e)


def _weather(location: str) -> str:
    try:
        resp = httpx.get("https://wttr.in/" + httpx.utils.quote(location) + "?format=4", timeout=8)
...
def _crypto_price(coin: str) -> str:
    try:
        symbol_map = {
            ...
            "doge": "dogecoin", "avax": "avalanche-2", "matic": "matic-network", "ltc": "litecoin",
        }
        ...
        if not data:
            return "Price not found for '" + coin + "'."
        price  = data.get("usd", "?")
        change = data.get("usd_24h_change")
        result = coin.upper() + ": $" + "{:,.4f}".format(price)
        if change is not None:
            result += " (24h: {:+.2f}%)".format(change)
        return result
    except Exception as e:
...
def _get_calendar(days: int = 7) -> str:
    ...
    try:
        ...
        for e in events:
            ...
            if "T" in start:
                ts = datetime.fromisoformat(start).astimezone(tz).strftime("%a %b %d, %I:%M %p")
            else:
                ts = start
            lines.append("- " + e.get("summary", "Untitled") + " (" + ts + ")")
        return "\n".join(lines)
    except Exception as e:
...
def _dispatch_tool(name: str, args: dict, chat_id: int, user_id: int, model: str) -> str:
    ...
    if name == "set_reminder":
        ...
        scheduler.add_job(
            ...
            id="rem_" + str(user_id) + "_" + str(run_at.timestamp()),
        )
        return "Reminder set for " + str(mins) + " min: '" + msg + "'"
...
async def _fire_reminder(chat_id: int, message: str):
    if _bot_app:
        try:
            await _bot_app.bot.send_message(chat_id=chat_id, text="Reminder: " + message)
        except Exception as e:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "I run multiple projects in parallel -- YouTube channels, backends, automations, scripts, research. "
        "I'll ask questions when I need info to get the job done.\n\n"
...
async def cmd_project(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not name:
        ...
        kb = [[InlineKeyboardButton(p, callback_data="switch_project:" + p)] for p in projects]
...
async def cmd_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in projects:
        if p == active:
            lines.append("> " + p + " (active)")
        else:
            count = len(user_state[uid]["projects"][p])
            lines.append("  " + p + " (" + str(count) + " messages)")
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Messages in context: " + str(msg_count) + "\n"
...
async def cmd_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Cleared project: " + proj)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("switch_project:"):
        name = data.split(":", 1)[1]
        set_active_project(uid, name)
        await query.edit_message_text("Switched to: " + name)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        history = get_history(uid)
        history.append({"role": "user", "content": "[Photo] " + caption})
        ...
        for _ in range(10):
            ...
            if msg.tool_calls:
                ...
                for tc in msg.tool_calls:
                    ...
                    logger.info("[%s] Tool: %s(%s)", get_active_project(uid), tc.function.name, args)
...
async def post_init(app: Application):
    ...
    _bot_app = app
    scheduler.start()
    logger.info("Bot v6 started -- port %s", PORT)
